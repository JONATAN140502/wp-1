...
class ExceptionalElements {
	...
	public function get_this_exceptional_element( $this_data, $attributes, $options ) {
		switch ( $this_data['name'] ) {
			...
			case 'pagination': {
				...
				if ( $show_pagination) {
					...
					if($pagination_type === 'numeric'){
						...
						} else if ($pagination_type === 'load_more' && $current_page !== $total_pages) {
			...
			case 'common': {
					...
					if ($type) {
						$attributes .= ' data-droip-type="' . $type . '"';
					}
	...
	private function collection_loading_element($this_data, $attributes, $options)
	{
		...
		return <<<EOF
			<$tag $attributes droip-collection="loading" data-element_hide="true">
				$children_markup
			</$tag>
		EOF;
	}
	...
	private function construct_collection_markup( $children, $this_data, $attributes, $element_name='collection' ) {
		...
		$id_attr = $element_name === 'slider' ? 'droip_slider_random_id' : 'droip_collection_random_id';
		$content_key = $element_name === 'slider' ? 'dynamicSliderContent' : 'dynamicContent';

		if (isset($this_data['properties'][$content_key]['collectionRandomId'])) {
			$attributes .= ' ' . $id_attr . '="' . $this_data['properties'][$content_key]['collectionRandomId'] . '"';
		}
	...
	private function image_element( $this_data, $attributes, $options ) {
		...
		if ($dynamic_content) {
			...
			if ($dynamic_content['type'] === 'reference' && isset($options['post'], $options['post']->ID)) {
				...
				$image_data = $content;
			} else if ($dynamic_content['type'] === 'user') {
	...
	private function slider_nav_element($this_data, $attributes, $options) {
		...
		$slider_nav_children = isset($this_data['children']) ? $this_data['children'] : [];
		$slider_mode = $options['slider_mode'] ?? 'manual';
