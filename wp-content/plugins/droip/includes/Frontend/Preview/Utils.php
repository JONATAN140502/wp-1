...
class Utils
{
	...
	private static function get_common_args($dynamic_content, $options)
	{
		...
		if (!empty($dynamic_content['type']) && false !== strpos($dynamic_content['type'], DROIP_CONTENT_MANAGER_PREFIX)) {
	...
	public static function getDynamicRichTextValue($dynamic_content, $options){
		...
		if ( $dynamic_content['type'] === 'post' ) {
			...
			} else if (('post_time' === $dynamic_content['value'] || 'time' === $cm_field_type) && isset($dynamic_content['timeFormat'])) {
