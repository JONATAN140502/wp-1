...
class HelperFunctions {
	 ...
	 * @param array $access_levels access levels => array(DROIP_ACCESS_LEVELS['FULL_ACCESS'], DROIP_ACCESS_LEVELS['CONTENT_ACCESS']);
	 *
	...
	public static function search_posts_by_query($name, $query, $post_parent, &$args) {
		...
		if (!str_contains($name, 'droip_cm_')) {
			return;
		}
			...
			if (in_array($data['type'], ['reference'], true)) {
				...
				if (!empty($matched_post_ids)) {
					...
					$reference_where_sql .= " OR {$wpdb->posts}.ID IN (
						SELECT post_id
						FROM {$wpdb->prefix}droip_cm_reference
	...
	public static function get_matched_post_ids_recursive($post_parent, $query, $depth = 0, $max_depth = 5) {
		...
		$post_type = 'droip_cm_' . $post_parent;
	...
	public static function get_session_id() {
		...
		if (isset($_COOKIE['droip_session_id'])) {
			self::$global_session_id = sanitize_text_field($_COOKIE['droip_session_id']);
			return self::$global_session_id;
		}
		...
		setcookie('droip_session_id', self::$global_session_id, time() + (DAY_IN_SECONDS * 7), COOKIEPATH, COOKIE_DOMAIN, is_ssl(), true);
