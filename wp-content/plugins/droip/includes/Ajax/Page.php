...
class Page {
	...
	public function format_single_post( $post_id ) {
		...
		if ( $post ) {
			...
			$temp_urls = HelperFunctions::get_post_url_arr_from_post_id( $post->ID );
    ...
    // Clean global styles
    $all_global_styles = HelperFunctions::get_global_data_using_key(DROIP_GLOBAL_STYLE_BLOCK_META_KEY) ?: [];
    [$all_global_styles, $changed] = $clean_styles($all_global_styles);
    if ($changed) {
    ...
    foreach ($all_post_ids as $p_id) {
        // Random styles
        $post_styles = get_post_meta($p_id, DROIP_GLOBAL_STYLE_BLOCK_META_KEY . '_random', true) ?: [];
        [$post_styles, $changed] = $clean_styles($post_styles);
        if ($changed) {
        ...
        if ($most_recent_stage_id) {
            foreach ([
                DROIP_GLOBAL_STYLE_BLOCK_META_KEY,
                DROIP_GLOBAL_STYLE_BLOCK_META_KEY . '_random'
            ] as $meta_key_base) {
                $meta_key = Staging::get_staged_meta_name($meta_key_base, $p_id, $most_recent_stage_id);
                $staged_styles = get_post_meta($p_id, $meta_key, true) ?: [];
                [$staged_styles, $changed] = $clean_styles($staged_styles);
                if ($changed) {
	...
	public static function get_unused_class_info_from_db($internal = false) {
		...
		$common_unused = array_values(array_filter($all_style_ids, function($id) use ($all_used_style_ids) {
	...
	private static function get_all_used_style_ids() {
		...
		foreach ($all_post_ids as $p_id) {
			// Published
			$published_used_ids = get_post_meta($p_id, DROIP_META_NAME_FOR_USED_STYLE_BLOCK_IDS, true) ?: [];
			$published_used_ids_random = get_post_meta($p_id, DROIP_META_NAME_FOR_USED_STYLE_BLOCK_IDS . '_random', true) ?: [];
			...
			if ($most_recent_stage_id) {
				$staged_used_keys = [
					Staging::get_staged_meta_name(DROIP_META_NAME_FOR_USED_STYLE_BLOCK_IDS, $p_id, $most_recent_stage_id),
					Staging::get_staged_meta_name(DROIP_META_NAME_FOR_USED_STYLE_BLOCK_IDS . '_random', $p_id, $most_recent_stage_id),
				];
				foreach ($staged_used_keys as $key) {
					$staged_used_ids = array_merge($staged_used_ids, get_post_meta($p_id, $key, true) ?: []);
				}
			}
